...
public class SM4 {
    ...
    public static final int PADDINGLENGTH = 0;
    public static final int PADDINGSPACE = 1;
    ...
    private byte[] paddingSpace(byte[] input, int mode) {
        ...
        if (mode == SM4_ENCRYPT) {
            ...
            for (int i = 0; i < p; i++) {
                ret[input.length + i] = (byte) 32;
            }
        } else {
            int p = 0;
            while (input[input.length - p - 1] == 32) ++p;
            ret = new byte[input.length - p];
    ...
    public byte[] sm4_crypt_ecb(SM4_Context ctx, byte[] input) throws Exception {
        ...
        if ((ctx.isPadding) && (ctx.mode == SM4_ENCRYPT)) {
            if (ctx.paddingMode == PADDINGLENGTH) {
                input = padding(input, SM4_ENCRYPT);
            } else if (ctx.paddingMode == PADDINGSPACE) {
                input = paddingSpace(input, SM4_ENCRYPT);
            }
        }
        ...
        if (ctx.isPadding && ctx.mode == SM4_DECRYPT) {
            if (ctx.paddingMode == PADDINGLENGTH) {
                ...
                output = paddingSpace(output, SM4_DECRYPT);
            }
        }
        bins.close();
        bous.close();
